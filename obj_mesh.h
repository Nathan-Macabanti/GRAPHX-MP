	...
	for (size_t m = 0; m < objData->materials.size(); m++) {
		...
		if (mp->diffuse_texname.length() > 0) {
			if (objData->textures.find(mp->diffuse_texname) == objData->textures.end()) {
				...
				std::string textureFileName = baseDir + "/" + "Sun.tga";
				//std::string textureFileName = baseDir + "/" + "Earth.tga";
				...
				//std::string textureFileName2 = baseDir + "/" + "Sun.tga";
				std::string textureFileName2 = baseDir + "/" + "Earth.tga";



				if (!FileExists(textureFileName2)) {
					std::cerr << "Unable to find file: " << textureFileName2 << std::endl;
					exit(1);
				}
				unsigned char* image2 = stbi_load(
				...
				if (!image2) {
					std::cerr << "Unable to load texture: " << textureFileName2 << std::endl;
					exit(1);
				}

				std::cout << "Loaded texture: " << textureFileName2 << std::endl;
				...
				objData->textures.insert(std::make_pair("Sun.tga", textureId2));
				...
				std::string textureFileName3 = baseDir + "/" + "Moon.tga";
				//std::string textureFileName = baseDir + "/" + "Earth.tga";



				if (!FileExists(textureFileName3)) {
					std::cerr << "Unable to find file: " << textureFileName2 << std::endl;
					exit(1);
				}
				unsigned char* image3 = stbi_load(
					textureFileName3.c_str(),
					&width3,
					&height3,
					&comp3,
					STBI_default
				...
				if (!image3) {
					std::cerr << "Unable to load texture: " << textureFileName3 << std::endl;
					exit(1);
				}

				std::cout << "Loaded texture: " << textureFileName3 << std::endl;


				glGenTextures(1, &textureId3);
				glBindTexture(GL_TEXTURE_2D, textureId3);

				GLenum format3 = GL_RGBA;
				if (comp3 == 3) {
					format3 = GL_RGB;
				}
				else if (comp3 == 4) {
				...
				objData->textures.insert(std::make_pair("Moon.tga", textureId3));
